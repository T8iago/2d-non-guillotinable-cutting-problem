...
void tikzRectangles(int n, rectangleSet *rect, FILE *file, int maxHeight,
                    int maxLength, double time, const char *fileName) {
  ...
  fprintf(file, "\n");
  fprintf(file, "\n");
  fprintf(file, "\\LARGE{Tempo: %f segundos}\n", time);
...
void executeFiles(FILE *finp, FILE *file, const char *fileName) {
    ...
    tikzRectangles(n, rect, file, maxHeight, minX, time, fileName);  // 0 para o tempo aqui, pois será passado na main
...
clock_t start, end;
    ...
    char pasta_saida[] = "./outputs";      // Pasta onde os arquivos de saída serão salvos

    // Criar a pasta de saída, se não existir
    struct stat st = {0};
    if (stat(pasta_saida, &st) == -1) {
        mkdir(pasta_saida, 0200);  // Permissões padrão para a pasta
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (entry->d_type == DT_REG) {  // Verifica se é um arquivo regular
            ...
            snprintf(caminho_saida, sizeof(caminho_saida), "%s/%s_out.tex", pasta_saida, entry->d_name);
