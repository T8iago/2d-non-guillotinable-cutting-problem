...
// int partitionX(intersectionSet *intersection, int low, int high) {
//   int pivot = intersection[high].x;
//   int i = (low - 1);

//   for (int j = low; j <= high - 1; j++) {
//     if (intersection[j].x <= pivot) {
//       i++;
//       swap(&intersection[i].x, &intersection[j].x);
//       swap(&intersection[i].y, &intersection[j].y);
//     }
//   }

//   swap(&intersection[i + 1].x, &intersection[high].x);
//   swap(&intersection[i + 1].y, &intersection[high].y);
//   return (i + 1);
// }

// void quickSortX(intersectionSet *intersection, int low, int high) {
//   if (low < high) {
//     int pi = partitionX(intersection, low, high);

//     quickSortX(intersection, low, pi - 1);
//     quickSortX(intersection, pi + 1, high);
//   }
// }

// int partitionY(intersectionSet *intersection, int low, int high) {
//   int pivot = intersection[high].y;
//   int i = (low - 1);

//   for (int j = low; j <= high - 1; j++) {
//     if (intersection[j].y <= pivot) {
//       i++;
//       swap(&intersection[i].x, &intersection[j].x);
//       swap(&intersection[i].y, &intersection[j].y);
//     }
...
// void quickSortY(intersectionSet *intersection, int low, int high) {
//   if (low < high) {
//     int pi = partitionY(intersection, low, high);

//     quickSortY(intersection, low, pi - 1);
//     quickSortY(intersection, pi + 1, high);
//   }
// }
...
int main() {
  ...
  // Open the input file
  FILE *finp = fopen("inputs/AAA.txt", "r");
  if (finp == NULL) {
  ...
  for (int i = 1; i < n + 1; i++) {
    //printf("Rectangle %d: (%d, %d)-(%d, %d)-(%d, %d)-(%d, %d)\n", i, rect[i].x, rect[i].y, rect[i].x + rect[i].l, rect[i].y, rect[i].x + rect[i].l, rect[i].y + rect[i].a, rect[i].x, rect[i].y + rect[i].a);
  }

  //printf("\n");
  for (int i = 1; i <= n; i++) {
    //printf("Edge %d: (%d, %d, %d, %d)\n", i, edge[i].leftX, edge[i].lowerY, edge[i].rightX, edge[i].upperY);
  }
